...
if grep -q "13.203.161.24:3001" src/services/preventiveHealthService.ts; then
    echo "✓ Code changes detected (IP address configured)"
else
    echo -e "${YELLOW}Warning: Code changes not found.${NC}"
    echo "The default URL should be: http://13.203.161.24:3001/api"
...
if [ -f ".env" ]; then
    if grep -q "AI_SERVICE_PYTHON_URL" .env; then
        ...
        echo "AI_SERVICE_PYTHON_URL=http://13.203.161.24:3001/api"
    else
...
if [ -f "ecosystem.config.js" ]; then
    ...
    if ! grep -q "AI_SERVICE_PYTHON_URL" ecosystem.config.js; then
        ...
        echo "Please verify it's set to: http://13.203.161.24:3001/api"
    fi
else
...
echo "   curl http://13.203.161.24:3001/health"
echo ""
echo "2. Test backend connection to AI service:"
echo "   curl http://13.203.161.24:3001/api/ai/preventive-health"
...
echo "   Check backend logs for: 'http://13.203.161.24:3001' (should see IP, not localhost)"
echo ""
